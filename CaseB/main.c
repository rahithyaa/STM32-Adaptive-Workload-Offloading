...
int main(void)
{
  ...
  DWT_Init();
    ...
    printf("[sim] Using synthetic vibration signal\r\n");

    HAL_TIM_Base_Stop_IT(&htim2);
    CostModel_Calibrate();
    g_T_local_predicted_us = (uint32_t)g_T_fft_calib_us;
    printf("[calib] T_fft_calib = %.1f us (corrected)\r\n", g_T_fft_calib_us);
    ...
    /* Tasks */
    osThreadDef(SafTask, SafetyTaskFunc,    osPriorityHigh,        0U, 256U);
    osThreadCreate(osThread(SafTask), NULL);

    osThreadDef(PrcTask, ProcessingTaskFunc, osPriorityNormal,     0U, 1024U);
    osThreadCreate(osThread(PrcTask), NULL);

    osThreadDef(DecTask, DecisionTaskFunc,  osPriorityNormal, 0U, 512U);
    osThreadCreate(osThread(DecTask), NULL);

    osThreadDef(MetTask, MetricsTaskFunc,   osPriorityNormal,         0U, 512U);
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim) {
    ...
    isrCount++;
    if(isrCount == 1000U) {
        printf("ISR firing OK\r\n");
    }*/

    /* Synthetic vibration: 50Hz dominant + 120Hz harmonic + noise */
